...
Hypothèses sur le fichier ../02_Data/PQ_BV_Cloutasse.csv :
  - séparateur ;
  - colonnes : dateP, dateQ, P_mm, Q_ls
"""
...
def detect_runoff_events(
    ...
    if P is not None and not isinstance(P, pd.Series):
        raise TypeError("P doit être une pd.Series si fourni.")
    ...
    if threshold is None:
        ...
        if len(q_pos) == 0:
            raise ValueError("Impossible d'estimer un seuil : Q ne contient que des NaN ou des zéros.")
        threshold = float(np.nanpercentile(q_pos, threshold_quantile))
        print(f"[INFO] Seuil automatique d'évènement (quantile {threshold_quantile}%) : {threshold:.3f} (unités de Q)")
    else:
    ...
    for s_core, e_core in zip(starts, ends):
        ...
        if P is not None:
            ...
            P_evt_series = pd.Series(
                data=np.nan, index=Q_evt.index, name="P_mm"
            )
...
def main():
    ...
    if Q_col is None:
        raise ValueError(
            "Impossible de trouver une colonne de débit dans le CSV.\n"
            f"Colonnes candidates testées : {candidate_cols}\n"
            f"Colonnes présentes : {list(df.columns)}"
        )
    print(f"Colonne de débit utilisée : {Q_col}")
    ...
    print("Détection des évènements de ruissellement...")
    ...
    for event_id, row in events_df.iterrows():
        ...
        ax1.plot(Q_plot.index, Q_plot.values, label="Q (obs)", linewidth=1.2)
        ...
        fig.suptitle(
            f"Évènement {event_id} : {t0:%Y-%m-%d %H:%M} → {t1:%Y-%m-%d %H:%M}",
            fontsize=10,
        )
        fig.tight_layout()

        fig_path = out_dir / f"event_{event_id:03d}.png"
